...
func TestGenericNumberValidator_ValidateFieldData(t *testing.T) {
	...
	// value < lt
	testValidateData[int](t, validator, &data, intoRawMessage(constraints)[0], false)

	// value > lt
	constraints.Lt = makePtr(64)
	testValidateData[int](t, validator, &data, intoRawMessage(constraints)[0], true)
...
func Test_numberTypeValidator_ValidateFieldSchema(t *testing.T) {
	validator := GenericNumberValidator[int]{}
	...
	testValidateSchema(t, &validator, id, intoRawMessage(constraints)[0], false)

	// unknown keys should not lead to errors
	m := make(map[string]interface{})
	m["invalid"] = 2
	testValidateSchema(t, &validator, id, intoRawMessage(m)[0], false)

	// wrong type
	m["lt"] = "hello"
	testValidateSchema(t, &validator, id, intoRawMessage(m)[0], true)
	...
	testValidateSchema(t, &validator, id, intoRawMessage(constraints)[0], true)
...
func testValidateData[T Number](t *testing.T, validator *GenericNumberValidator[T], data *model.WebFormDataRaw, constraints json.RawMessage, expectErr bool) {
	err := validator.ValidateFieldData(data, &intoRawMessage(constraints)[0])
	if expectErr {
...
func testValidateSchema[T Number](t *testing.T, validator *GenericNumberValidator[T], id int, constraints json.RawMessage, expectErr bool) {
	err := validator.ValidateFieldSchema(id, &intoRawMessage(constraints)[0])
	if expectErr {
		assert.False(t, err.IsEmpty())
		assert.Len(t, err.FailedConstraints, 1)
	} else {
